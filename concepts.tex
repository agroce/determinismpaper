...
{\bf If a call \emph{fails}, and indicates this to the caller, it should not modify system
  state in any way; changes should be ``rolled back.''} 
...
nondeterminism captures such issues as the Apple login bug.  The
changes to visible or hidden state we are most concerned with are
usually those related to the failure itself.
...
The concept of an action does not restrict this approach to reactive
...
Note that vertical nondeterminism may, in theory, exhibit only
...
not idempotent.  Only the existence of \emph{non-visible} state changes that
...
whether the action $A$ \emph{failed}.  
...
\section{Sources of Nondeterminism in Practice}
\label{sec:sources}
...
simply executing the same test twice in succession, without attempting
...
process will not reveal the problem.  In terms of our formalism, an
...
what happens when $\mathit{PRED}$ not deterministic.
