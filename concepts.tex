...
\xrightarrow[]{\alpha_{n-1}} s_n$ where $s_0 \in I$ and $\forall i
...
\xrightarrow[]{\alpha^2_{n-1}} s^2_n$ are said to show \emph{visible value
  determinism} if $\exists i > 0$ such that: $\forall j < i . \alpha^1_j = \alpha^2_j \wedge
...
$\alpha_i \in \mathcal{I} \wedge \alpha_i = \alpha_{i+1}$ and $v(s_i)
\neq v(s_{i+2})$.
...
$\alpha_i = \alpha_{i+1} \wedge \neg F_{i+2}$.  The second possibility
captures our more lightweight approach to checking for failure nondeterminism.  Note that unlike all of the other ways to demonstrate nondeterminism, this second type of failure nondeterminism does not require the use of $v$, or even the notion of observable components of a state, or (for that matter) any comparison of states.
