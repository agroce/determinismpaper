...
primary sources, and most of which can be detected easily
...
modified binary search, but the
following discussion relies only on the general structure of
delta-debugging \cite{CReduce,onetest}, which is as follows.  Given a
...
function calls.  This is the typical use of
...
\cite{icst2014,stvrcausereduce,NonAdeq}.  Our contribution is to consider
...
In \emph{monotonic} probabilistic delta-debugging, removing part of a test \emph{cannot
...
\emph{increase} the probability that $t$ behaves nondeterministically.
\begin{comment}
...
\end{comment}
In probabilistic
settings, we may want to exploit non-monotonicity to find a
subset of a test where a predicate that is very far from holding (in terms of probability, $P$) on the original test now
holds.  For instance, a test may be very infrequently nondeterministic, and we want to find a subset where there is much greater probability of variance.
...
such as $P(fail) > 0.3 \wedge P(fail) < 0.7$, to force a test to be highly flaky, and force it to fail
...
space that often contains thousands or even millions of
...
produce a very poor estimate of $P(fail)$.  At this point, delta-debugging is
``stuck'' with the error, because test reduction
...
the actual probability constraints of $\mathit{PRED}$ are not satsified.  False negatives
also exist, but are usually harmless, since it often just forces a different, equally
desirable reduction path.
...
0.05$ (that is, by some statistical test, the probability
...
modifying delta-debugging implementations, which is difficult and
...
should be implementable simply by modifying $\mathit{PRED}$.

A costly but somewhat effective solution is to make $N$ large in comparison to
...
and 0.10, respectively, storing the value of the operation in one of five array locations.
