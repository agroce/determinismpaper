...
\vspace{0.1in}
<int> := <[-10..10]>
<int> := storage.getNewVal()
\{KeyError,ValueError\} storage.store(<int>)
\vspace{0.1in}
property: len(storage.contents()) == len(set(storage.contents()))
\end{code}
...
Python library and test harness with both vertical and horizontal nondeterminism.