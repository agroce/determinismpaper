...
We implemented our approach in the TSTL \cite{NFM15,ISSTA15,tstlsttt} system, a
...
We therefore instead made nondeterminism detection a first-class
...
a parameter, or even due to a shared reference held by two values).
...
comparison, but only on the final values of all pool variables after a test has
finished executing.
...
in reducing the tests for the {\tt redis-py} example discussed below).
...
Because of the idiosyncracies of afl instrumentation and process
