...
delta-debugging for real nondeterministic predicates.  We chose two of
...
generation effort.  The other two subjects were chosen, and harnesses
...
nondeterminism.  The overhead on testing imposed by nondeterminism
checks varied considerably, but in the worst case did not double the
average cost of each action, and thus never halved the amount of
testing performed.  Test reduction always reduced the size of
generated tests by 90\% or more, thus showing that effective reduction
has a large payoff.
...
\hline
Parallel Sort & Yes (see below) & $\sim$40\% & & $\sim$2 minutes\\
...
a set of minimal nondeterministic tests exposing the sources of
nondeterminism/flakiness.  This process is unlikely to require 12
hours, of course; our point is that it is completely automated,
including generation of a set of minimal, distinct, tests showing
different kinds of nondeterminism using test normalization~\cite{onetest}.  The actual amount of time given
...
remove nondeterminism.  We determined that the mean time to find all
the sources of nondeterminism and produce minimal tests 3.2
hours, which is not trivial, but also, for this purpose, not excessive.
