...
points we wanted to explore were (1) whether lightweight, automatic,
nondeterminism detection was able to reliably detect actual
...
\caption{Standard delta-debugging (runtime 38s)}
\label{fig:r1}
\end{subfigure}
\begin{subfigure}{0.67\columnwidth}
\centering
\includegraphics[width=\columnwidth]{redisforcep}
\caption{P=0.5, 100 samples (runtime 2131s)}
\label{fig:r2}
\end{subfigure}
\begin{subfigure}{0.67\columnwidth}
\centering
\includegraphics[width=\columnwidth]{redisforceprep}
\caption{P=0.5, 10 samples, 10 replications (runtime 617s)}
\label{fig:r3}
\end{subfigure}
...
behavior.   Using {\tt tstl\_afl\_fuzz}, we can see that the {\tt redis-py} has a 
...
a more reliably nondeterministic test.  
...
text I/O on output pipes, rather than values in a Python dictionary.
